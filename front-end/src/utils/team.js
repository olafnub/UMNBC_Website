class Officer {
    constructor(img, alt, name, title) {
        this.img = img
        this.alt = alt
        this.name = name
        this.title = title
    }
}


let officers = [
    new Officer(
        "https://oh.imgix.net/wp-content/uploads/2022/01/pudgy-penguin-3.jpg?auto=format%2Ccompress&ixlib=php-3.3.0&w=400",
        "Pudgy penguin NFT",
        "Jorgen",
        "Co President"
    ),
    new Officer(
        "https://i.redd.it/pc6kk3p0rjf81.png",
        "Etemon NFT",
        "Andy",
        "Co President"
    ),
    new Officer(
        "https://cdnb.artstation.com/p/assets/images/images/048/075/205/large/imdmitreey-40.jpg?1649153616",
        "floating reindeer head NFT",
        "Anthony",
        "Social Media"
    ),
    new Officer(
        "https://www.artnews.com/wp-content/uploads/2021/08/BAYC-8746.png",
        "BAYC NFT shooting laser eyes",
        "Anjali",
        "Social Media"
    ),
    new Officer(
        "https://nftcalendar.io/storage/uploads/news/2022/1/iTUhqA1rG1qxWxMUCJqJRn9xRVc6nIG9Rp7uPopc.jpg",
        "Animal drawing with a blue backpack profile picture with a colorful background",
        "Sofia",
        "Education"
    ),
    new Officer(
        "https://hellopartner.com/content/images/size/w1600/uz4FXC4Ws_hBBlHGegO5-FkwU9TFA1973dYtW2qZfbJXE-XgRgVy3I8frM-P2HLhBmmRgtHdUL9sK9aIr4ZjF2u8PIqqrZSO6hRBIfk-MNyHksdB2sMWGntaHAx5kHOGTa_ZcNqG.png",
        "Smoking pink bear NFT",
        "Abdi",
        "Treasure"
    ),
    new Officer(
        "https://assets-global.website-files.com/65217fd9e31608b8b68141ba/65217fd9e31608b8b6814490_fQ7W42deNUL4fV_shUyIIVyt9c7RoIUU5gx4oVv9MvQJNLNY6nxBrLBcAv5aJ0jVcppwNLvut5I-ALAT8csQ-cQaJEuPgURTkkX0gg5cFos33USpISZ7PJ_OsHd1WoiwqvuxyYED9Jn0J2X23jEnw9k.png",
        "Bored Ape nft with a cowboy hat and blindfolded",
        "Veeraj",
        "Web Developer"
    ),
    new Officer(
        "https://images.lifestyleasia.com/wp-content/uploads/sites/6/2022/01/14121356/mutant-975x1024-1-1.jpeg",
        "Cyborg monkey shooting diamond out of eyes",
        "Adhithi",
        "Web Developer"
    ),
    new Officer(
        "https://assets-global.website-files.com/6241bcd9e666c1514401461d/63a0c5d4a82cab020d4015df_solanredpandas%20upcomingnftdrop%20nftmintradar%20solanadrop.png",
        "Samurai panda with sun glasses",
        "Prashant",
        "Outreach Coordinator"
    ),
    new Officer(
        "https://oh.imgix.net/wp-content/uploads/2022/01/pudgy-penguin-3.jpg?auto=format%2Ccompress&ixlib=php-3.3.0&w=400",
        "Skeleton holding wand NFT",
        "Ahmed",
        "Officer"
    ),
    new Officer(
        "https://cdnb.artstation.com/p/assets/images/images/048/075/227/large/imdmitreey-31.jpg?1649153670",
        "Penguin with durag NFT",
        "Jackie",
        "Officer"
    ),
    new Officer(
        "https://dam.sothebys.com/dam/image/lot/7c364e54-f062-4358-88f6-194cac189b58/primary/extra_small",
        "Invisbile Friend, basketball, potion, hat, and rainbow sunglasses",
        "Daigan",
        "Web Developer"
    ),
    new Officer(
        "https://pbs.twimg.com/media/FbkuRH7VsAE1EzV?format=jpg&name=4096x4096",
        "Zebra with tie and has cool sunglasses which are purple and green shades",
        "Aryan",
        "Education"
    ),
];

export default officers;